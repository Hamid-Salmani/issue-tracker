...
const NavBar = () => {
  ...
  return (
    <nav className="sticky top-0 z-50 backdrop-blur-md bg-white/60 border-b border-white/30 shadow-md px-6 py-4 flex items-center justify-between">
      <Link href="/" className="flex items-center space-x-2 text-sky-600 hover:text-sky-700 transition-colors">
          ...
          <li key={link.href}>
            <Link
              ...
              className={classnames(
                ...
                {
                'text-sky-700 bg-white/80 shadow-sm': CurrentPathName.startsWith(link.href),
                'text-gray-600 hover:text-sky-600': !CurrentPathName.startsWith(link.href),
                }
              )}
            >
              {link.name}
